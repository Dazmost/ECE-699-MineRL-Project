...
workspace_path= '/home/endeavor/projects/rrg-mcrowley/endeavor/minecraft_bot_dev_A2C_final_100neuron_dense'
data_path= '/home/endeavor/projects/def-mcrowley/endeavor'
env_name = 'MineRLTreechop'
gpu_use = True

pretrained_model = False
pretrained_name = 'tree_supervised_model_10'
num_epochs = 2 #epochs per video #4
iter_all_eps = 10 #number of times train on all 82 training videos/episodes
num_actions = 32
num_frames = 5
...
def main():
    ...
    train_log_dir = workspace_path + '/logs/gradient_tape/' + current_time + '/train'
    ...
    optimizer = tf.keras.optimizers.Adam(learning_rate=0.0005, epsilon=0.01) #learning rate was 0.0005
    
    # Prepare the metrics.
    train_actor_loss_metric = tf.keras.metrics.Mean('actor_loss', dtype=tf.float32)
    train_critic_loss_metric = tf.keras.metrics.Mean('critic_loss', dtype=tf.float32)
    train_accuracy_metric = tf.keras.metrics.CategoricalAccuracy('train_accuracy')
...
#             replay_obs_lstm = frame_partition(replay_obs_list)
        ...
        # MODEL TRAINING START
                    ...
                    with tf.GradientTape() as tape:
                        ...
                        prediction, value = convlstm_model(tf.expand_dims(obs,0), training=True) 
                        ...
                        act_loss = cce_loss_logits(replay_act_array_onehot, prediction)
                        c_loss = 0.5*tf.keras.losses.mean_squared_error(reward_array, v)

                        tf.print("act_loss: ", act_loss)
                        regularization_loss = tf.reduce_sum(convlstm_model.losses)
                        total_loss = act_loss + 1e-5 * regularization_loss

                    grads = tape.gradient([total_loss, c_loss], convlstm_model.trainable_variables) #[total_loss, None]
                    optimizer.apply_gradients(zip(grads, convlstm_model.trainable_variables))

                    print("actor_loss: ", total_loss)
                    print("critic_loss: ", c_loss)
                    print("")
                    
                    # Update training metric.###################HERE#######################
                    # act_probs_onehot = tf.one_hot(tf.math.argmax(act_probs,1), num_actions) 
                    train_actor_loss_metric.update_state(total_loss)
                    train_critic_loss_metric.update_state(c_loss)
                    train_accuracy_metric.update_state(replay_act_array_onehot, prediction) # or act_probs or act_probs_onehot
                ...
                train_actor_loss = train_actor_loss_metric.result()
                print("Training actor loss over epoch: %.4f" % (float(train_actor_loss),))
                train_critic_loss = train_critic_loss_metric.result()
                print("Training critic loss over epoch: %.4f" % (float(train_critic_loss),))
                ...
                if (current_epoch_vid+1 == num_epochs): #after training on a video (after all epochs per vid) it saves loss and accuracy
                    ...
                    with train_summary_writer.as_default(): 
                        tf.summary.scalar('actor_loss', train_actor_loss_metric.result(), step=count)
                        tf.summary.scalar('critic_loss', train_critic_loss_metric.result(), step=count)
                        tf.summary.scalar('accuracy', train_accuracy_metric.result(), step=count)
                ...
                train_actor_loss_metric.reset_states()
                train_critic_loss_metric.reset_states()
